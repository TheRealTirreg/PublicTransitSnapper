...
def find_route_name(network, route, dist=0.05):
    """
    Return the most likely path and line as a dict.
    ...
    if route and route[0] != '0, 0, 0':
        ...
        if not path:
            for i in range(len(route)):
                ...
                path = calculate_path(network, route_shorter, dist)[1:-1]
                if path:
                    break
        """
    ...
    if not path:
        return {"edges": [[]], "path": [], "route_name": "4",
...
def get_all_data(location, network, path, shape_id, service_id, trip_id, route_id):
    ...
    start_time = time()
    # get a possible path from the graph with the matched edges
    ...
    for i in range(len(path_coords) - 1):
        ...
        if start == end:
            continue
        # if not on the same shape then do not calculate path
    ...
    ret["path"] = matched_path + [(probable_location.x, probable_location.y)]
    end_time = time()
